...
async def get_company_detail_by_id(db: AsyncSession, company_id: UUID) -> CompanyDetailResponse:
    ...
    return CompanyDetailResponse(
        ...
        invoice_sent_date=next((i.invoice_date for i in order.invoices if i.is_sent), None) if order else None,
        invoice_paid_date=next((i.invoice_date for i in order.invoices if i.is_paid), None) if order else None,
